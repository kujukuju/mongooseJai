...
EXPORT bool mg_split(struct mg_str *s, struct mg_str *k, struct mg_str *v, char delim);
...
EXPORT bool mg_path_is_sane(const char *path);
...
EXPORT void mg_queue_init(struct mg_queue *, char *, size_t);        // Init queue
EXPORT size_t mg_queue_book(struct mg_queue *, char **buf, size_t);  // Reserve space
EXPORT void mg_queue_add(struct mg_queue *, size_t);                 // Add new message
EXPORT size_t mg_queue_next(struct mg_queue *, char **);  // Get oldest message
EXPORT void mg_queue_del(struct mg_queue *, size_t);      // Delete oldest message
...
EXPORT size_t mg_vxprintf(void (*)(char, void *), void *, const char *fmt, va_list *);
EXPORT size_t mg_xprintf(void (*fn)(char, void *), void *, const char *fmt, ...);
...
EXPORT size_t mg_snprintf(char *, size_t, const char *fmt, ...);
EXPORT char *mg_vmprintf(const char *fmt, va_list *ap);
EXPORT char *mg_mprintf(const char *fmt, ...);
EXPORT size_t mg_queue_vprintf(struct mg_queue *, const char *fmt, va_list *);
EXPORT size_t mg_queue_printf(struct mg_queue *, const char *fmt, ...);

// %M print helper functions
EXPORT size_t mg_print_base64(void (*out)(char, void *), void *arg, va_list *ap);
EXPORT size_t mg_print_esc(void (*out)(char, void *), void *arg, va_list *ap);
EXPORT size_t mg_print_hex(void (*out)(char, void *), void *arg, va_list *ap);
EXPORT size_t mg_print_ip(void (*out)(char, void *), void *arg, va_list *ap);
EXPORT size_t mg_print_ip_port(void (*out)(char, void *), void *arg, va_list *ap);
EXPORT size_t mg_print_ip4(void (*out)(char, void *), void *arg, va_list *ap);
EXPORT size_t mg_print_ip6(void (*out)(char, void *), void *arg, va_list *ap);
EXPORT size_t mg_print_mac(void (*out)(char, void *), void *arg, va_list *ap);

// Various output functions
EXPORT void mg_pfn_iobuf(char ch, void *param);  // param: struct mg_iobuf *
EXPORT void mg_pfn_stdout(char c, void *param);  // param: ignored
...
EXPORT void mg_log(const char *fmt, ...);
EXPORT bool mg_log_prefix(int ll, const char *file, int line, const char *fname);
EXPORT void mg_log_set(int log_level);
EXPORT void mg_hexdump(const void *buf, size_t len);
EXPORT void mg_log_set_fn(mg_pfn_t fn, void *param);
...
EXPORT void mg_timer_init(struct mg_timer **head, struct mg_timer *timer,
                   uint64_t milliseconds, unsigned flags, void (*fn)(void *),
                   void *arg);
EXPORT void mg_timer_free(struct mg_timer **head, struct mg_timer *);
EXPORT void mg_timer_poll(struct mg_timer **head, uint64_t new_ms);
EXPORT bool mg_timer_expired(uint64_t *expiration, uint64_t period, uint64_t now);
...
EXPORT extern struct mg_fs mg_fs_packed;  // Packed FS, see examples/device-dashboard
...
EXPORT char *mg_random_str(char *buf, size_t len);
...
EXPORT uint64_t mg_millis(void);
...
struct mg_addr;
EXPORT int mg_check_ip_acl(struct mg_str acl, struct mg_addr *remote_ip);
...
EXPORT int mg_iobuf_init(struct mg_iobuf *, size_t, size_t);
EXPORT int mg_iobuf_resize(struct mg_iobuf *, size_t);
EXPORT void mg_iobuf_free(struct mg_iobuf *);
EXPORT size_t mg_iobuf_add(struct mg_iobuf *, size_t, const void *, size_t);
...
EXPORT struct mg_connection *mg_connect(struct mg_mgr *, const char *url,
                                 mg_event_handler_t fn, void *fn_data);
EXPORT struct mg_connection *mg_wrapfd(struct mg_mgr *mgr, int fd,
                                mg_event_handler_t fn, void *fn_data);
...
EXPORT int mg_mkpipe(struct mg_mgr *, mg_event_handler_t, void *, bool udp);
...
// Utility functions
EXPORT struct mg_timer *mg_timer_add(struct mg_mgr *mgr, uint64_t milliseconds,
                              unsigned flags, void (*fn)(void *), void *arg);

// Low-level IO primives used by TLS layer
enum { MG_IO_ERR = -1, MG_IO_WAIT = -2, MG_IO_RESET = -3 };
EXPORT long mg_io_send(struct mg_connection *c, const void *buf, size_t len);
EXPORT long mg_io_recv(struct mg_connection *c, void *buf, size_t len);
...
EXPORT struct mg_str mg_http_var(struct mg_str buf, struct mg_str name);
...
EXPORT long mg_http_upload(struct mg_connection *c, struct mg_http_message *hm,
...
EXPORT void mg_hello(const char *url);
...
EXPORT void mg_tls_init(struct mg_connection *, const struct mg_tls_opts *);
...
EXPORT size_t mg_tls_pending(struct mg_connection *);
...
EXPORT struct mg_connection *mg_ws_connect(struct mg_mgr *, const char *url,
                                    mg_event_handler_t fn, void *fn_data,
                                    const char *fmt, ...);
EXPORT void mg_ws_upgrade(struct mg_connection *, struct mg_http_message *,
                   const char *fmt, ...);
EXPORT size_t mg_ws_send(struct mg_connection *, const void *buf, size_t len, int op);
EXPORT size_t mg_ws_wrap(struct mg_connection *, size_t len, int op);
EXPORT size_t mg_ws_printf(struct mg_connection *c, int op, const char *fmt, ...);
EXPORT size_t mg_ws_vprintf(struct mg_connection *c, int op, const char *fmt,
                     va_list *);
...
EXPORT struct mg_connection *mg_sntp_connect(struct mg_mgr *mgr, const char *url,
                                      mg_event_handler_t fn, void *fn_data);
EXPORT void mg_sntp_request(struct mg_connection *c);
...
EXPORT struct mg_connection *mg_mqtt_listen(struct mg_mgr *mgr, const char *url,
                                     mg_event_handler_t fn, void *fn_data);
EXPORT void mg_mqtt_login(struct mg_connection *c, const struct mg_mqtt_opts *opts);
EXPORT void mg_mqtt_pub(struct mg_connection *c, const struct mg_mqtt_opts *opts);
EXPORT void mg_mqtt_sub(struct mg_connection *, const struct mg_mqtt_opts *opts);
EXPORT int mg_mqtt_parse(const uint8_t *, size_t, uint8_t, struct mg_mqtt_message *);
...
EXPORT void mg_mqtt_disconnect(struct mg_connection *, const struct mg_mqtt_opts *);
EXPORT size_t mg_mqtt_next_prop(struct mg_mqtt_message *, struct mg_mqtt_prop *,
                         size_t ofs);
...
EXPORT int mg_json_get(struct mg_str json, const char *path, int *toklen);

EXPORT bool mg_json_get_num(struct mg_str json, const char *path, double *v);
EXPORT bool mg_json_get_bool(struct mg_str json, const char *path, bool *v);
EXPORT long mg_json_get_long(struct mg_str json, const char *path, long dflt);
EXPORT char *mg_json_get_str(struct mg_str json, const char *path);
EXPORT char *mg_json_get_hex(struct mg_str json, const char *path, int *len);
EXPORT char *mg_json_get_b64(struct mg_str json, const char *path, int *len);

EXPORT bool mg_json_unescape(struct mg_str str, char *buf, size_t len);
...
EXPORT void mg_rpc_add(struct mg_rpc **head, struct mg_str method_pattern,
                void (*handler)(struct mg_rpc_req *), void *handler_data);
EXPORT void mg_rpc_del(struct mg_rpc **head, void (*handler)(struct mg_rpc_req *));
EXPORT void mg_rpc_process(struct mg_rpc_req *);

// Helper functions to print result or error frame
EXPORT void mg_rpc_ok(struct mg_rpc_req *, const char *fmt, ...);
EXPORT void mg_rpc_vok(struct mg_rpc_req *, const char *fmt, va_list *ap);
EXPORT void mg_rpc_err(struct mg_rpc_req *, int code, const char *fmt, ...);
EXPORT void mg_rpc_verr(struct mg_rpc_req *, int code, const char *fmt, va_list *);
EXPORT void mg_rpc_list(struct mg_rpc_req *r);
